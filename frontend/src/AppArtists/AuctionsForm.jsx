...
function AuctionsForm({ onAddAuction }) {
    ...
    const handleSubmit = async (e) => {
        ...
        form.append('timeRemaining', formData.timeRemaining);
        form.append('highestBid', formData.highestBid);
        form.append('endTime', formData.endTime); // إرسال وقت انتهاء المزاد
        if (formData.image) {
            form.append('image', formData.image);
        }

        try {
            const response = await axios.post('http://localhost:3001/api/auctions', form, {
        ...
        <div className="flex items-center justify-center mt-4">
            <div className="flex-col p-6 rounded-lg shadow-md mb-6 w-[90vw] sm:w-[80vw] md:w-[60vw] lg:w-[40vw] xl:w-[30vw]">
                ...
                <form onSubmit={handleSubmit} className='text-right'>
                    <label className='m-1'>اضافة وقت الانتهاء</label>
                <input
                        type="text"
                        placeholder="متبقي 6 سعات من الان علي انتهاء المزداد"
                        value={formData.timeRemaining}
                        onChange={handleChange}
                        name="timeRemaining"
                        className="mb-2 w-full border border-gray-300 p-2  rounded-md"
                    />
                    <label className='m-1'> اضافة نوع المزاد</label>

                        <input
                        type="text"
                        placeholder="مزاد علني"
                        value={formData.speciality}
                        onChange={handleChange}
                        name="speciality"
                        className="mb-2 w-full border border-gray-300 p-2 rounded-md"
                    />
                    <label className='m-1'>اكتب عنوان المزاد</label>
                    ...
                    <input
                        ...
                        className="mb-2 w-full border border-gray-300 p-2 rounded-md"
                    />
            <label className='m-1'> اكتب تفاصيل العمل الفني</label>
                    ...
                    <input
                        ...
                        className="mb-2 w-full border border-gray-300 p-2 rounded-md"
                    />
                    <label className='m-1'> اضافة السعر</label>
